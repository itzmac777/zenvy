...
export function TurfBookingClient({ turf }: { turf: BookingField }) {
  ...
  return (
    <>
      <a href="#slot-selection" aria-label="Jump to available slots" className="fixed bottom-5 right-5 z-40 grid h-12 w-12 place-items-center rounded-full border border-line bg-[#fffefd]/95 text-ink shadow-[0_16px_38px_rgba(23,22,19,0.18)] backdrop-blur-md transition hover:-translate-y-1 md:hidden"><Icon name="arrow-down" className="h-5 w-5 animate-bounce" /></a>
      <aside id="booking" className="min-w-0 scroll-mt-[112px] lg:sticky lg:top-[104px] lg:col-start-2 lg:row-span-2 lg:row-start-1 lg:self-start">
        <div className="min-w-0 border border-line bg-[#fffefd]/95 p-5 shadow-[0_24px_70px_rgba(23,22,19,0.08)] md:p-7">
          ...
          <form className="mt-6 grid min-w-0 gap-6" onSubmit={handleSubmit}>
            <fieldset><legend className="flex w-full items-center justify-between gap-3 text-sm font-bold"><span>Select a date</span><span className="font-normal text-muted">Next 7 days</span></legend><div className="scrollbar-none -mx-1 mt-3 flex max-w-full gap-2 overflow-x-auto px-1 pb-1">{(availability?.days ?? Array.from({ length: 7 }, (_, index) => ({ date: addDays(today, index), slots: [] }))).map((day) => { const meta = dateMeta(day.date, today); const active = day.date === selectedDate; return <button key={day.date} type="button" aria-pressed={active} onClick={() => selectDay(day.date)} className={`grid min-h-[68px] min-w-[64px] place-items-center border px-2 text-center ${active ? "border-olive bg-olive text-white" : "border-line bg-white text-ink hover:border-[#aba397]"}`}><span className="text-[10px] font-bold uppercase">{meta.weekday}</span><span className="font-serif text-xl leading-none">{meta.date}</span><span className={`text-[10px] ${active ? "text-white/75" : "text-muted"}`}>{meta.month}</span></button>; })}</div></fieldset>
            <fieldset id="slot-selection" className="scroll-mt-[92px]"><legend className="flex w-full items-center justify-between gap-3 text-sm font-bold"><span>Available one-hour slots</span><span className="font-normal text-muted">{turf.openingHours}</span></legend><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">{loading ? <p className="col-span-full py-6 text-center text-sm text-muted">Loading live slots...</p> : null}{!loading && selectedDay?.slots.length === 0 ? <p className="col-span-full border border-line bg-panel p-4 text-center text-sm text-muted">No slots are open on this date.</p> : null}{selectedDay?.slots.map((slot) => { const unavailable = slot.status !== "AVAILABLE"; const selected = selectedStarts.includes(slot.startAt); return <button key={slot.startAt} type="button" disabled={unavailable} aria-pressed={selected} onClick={() => selectSlot(slot)} className={`min-h-[58px] border px-2 py-2 text-left transition ${selected ? "border-olive bg-[#eef0ff] text-olive-dark" : unavailable ? "cursor-not-allowed border-line/70 bg-panel/50 text-muted/55" : "border-line bg-white hover:border-olive/55 hover:bg-[#f7f7ff]"}`}><span className="block text-xs font-bold">{slot.label}</span><span className="mt-1 block text-[10px]">{unavailable ? "Unavailable" : selected ? "Selected" : `${formatMoney(slot.priceBdt)}/hr`}</span></button>; })}</div><p className="mt-3 text-xs leading-relaxed text-muted">Select one hour, or extend into adjacent open hours for longer games.</p></fieldset>
            <fieldset><legend className="text-sm font-bold">Payment amount</legend><div className="mt-3 grid grid-cols-2 border border-line bg-panel/60 p-1">{[{ value: "advance" as const, label: "10% advance", amount: Math.ceil(total * 0.1) }, { value: "full" as const, label: "Full amount", amount: total }].map((option) => <button key={option.value} type="button" aria-pressed={paymentMode === option.value} onClick={() => setPaymentMode(option.value)} className={`min-h-[58px] px-3 text-left text-sm font-bold ${paymentMode === option.value ? "bg-white text-ink shadow-sm" : "text-muted"}`}><span className="block">{option.label}</span><span className="mt-1 block text-[11px] font-normal">{total ? formatMoney(option.amount) : "Select slots first"}</span></button>)}</div></fieldset>
